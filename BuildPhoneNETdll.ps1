# Name:    BuildPhoneNETdll
# Purpose: Compile the mdPhoneNET.dll wrapper
...
Write-Host "`n======================= WELCOME TO MELISSA PHONE OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdPhoneNET.dll -platform:AnyCPU -target:library mdPhone_cSharpCode.cs" > $null

if (Test-Path ("mdPhoneNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdPhoneNET.dll"

  Write-Host "`nmdPhoneNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdPhoneNET.dll was not created successfully`n"
